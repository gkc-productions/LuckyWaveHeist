...
local fontRegular = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium)
local fontBold = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold)
local fontBlack = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Heavy)
...
coinsLabel.FontFace = fontRegular
...
roundStatus.FontFace = fontRegular
...
roundMeta.FontFace = fontRegular
...
local buttons = {
	...
	NimbleFeet = makeButton(112),
	HighJumper = makeButton(148),
	BreakEfficiency = makeButton(184),
...
tutorialFrame.FontFace = fontRegular
...
local function refreshShop()
	for key, btn in pairs(buttons) do
		local def = Content.Shop[key]
		if def then
			local owned = upgradeLevels[key]
			if owned then
				btn.Text = string.format("%s (OWNED)", key)
				btn.AutoButtonColor = false
			else
				btn.Text = string.format("%s (%d)", key, def.cost)
				btn.AutoButtonColor = currentCoins >= def.cost
			end
		end
	end
end
...
roundEvt.OnClientEvent:Connect(function(p)
	...
	local waveDuration = Content.Tuning.WaveDurations[wave] or 1
	local progress = 0
	if state == "Wave" then
